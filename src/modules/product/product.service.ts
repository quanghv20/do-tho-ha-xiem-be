    ...
    async validateUnique(dto: Partial<Product>, excludeId?: number) {
        if (dto.code) {
            const exists = await this.productRepo.findOne({
                where: { code: dto.code, ...(excludeId ? { id: Not(excludeId) } : {}) } as any,
            });
        ...
        if (dto.name) {
            const exists = await this.productRepo.findOne({
                where: { name: dto.name, ...(excludeId ? { id: Not(excludeId) } : {}) } as any,
            });
        ...
        if (dto.sortOrder !== undefined) {
            const exists = await this.productRepo.findOne({
                where: { sortOrder: dto.sortOrder, ...(excludeId ? { id: Not(excludeId) } : {}) } as any,
            });
            if (exists) throw new BadRequestException('Thứ tự hiển thị đã tồn tại.');
        }
    }

    async create(dto: CreateProductDto): Promise<Product | any> {
