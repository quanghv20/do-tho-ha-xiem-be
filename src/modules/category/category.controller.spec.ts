...
describe('CategoryController', () => {
    ...
    it('Phải trả về mảng danh mục', async () => {
        const result = [{ id: 1, code: 'A', name: 'DanhMucA', description: '', sortOrder: 1 }];
    ...
    it('Phải tạo được danh mục mới', async () => {
        const dto: CreateCategoryDto = { code: 'B', name: 'DanhMucB', description: '', sortOrder: 2 };
    ...
    it('Phải cập nhật được danh mục', async () => {
        ...
        mockCategoryService.update.mockResolvedValue({ id: 1, code: 'A', name: 'DanhMucDaCapNhat', description: '', sortOrder: 1 });

        expect(await controller.update(1, dto)).toEqual({ id: 1, code: 'A', name: 'DanhMucDaCapNhat', description: '', sortOrder: 1 });
